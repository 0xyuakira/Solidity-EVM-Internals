...
contract SlotWidthTest is Test {
    ...
    function test_bytes32_fits_single_slot() public {
        ...
        bytes32 slot0 = vm.load(address(slotContract), bytes32(slotContract.dataSlot()));
    ...
    function test_bytes33_spills_into_two_slots() public {
        ...
        bytes32 slot1 = vm.load(address(slotContract), bytes32(slotContract.dataSlot()));
        for (uint256 i = 0; i < 32; i++) {
            assertEq(slot1[i], b33[i]);
        }

        // Load the second slot containing the last byte
        bytes32 slot2 = vm.load(address(slotContract), bytes32(slotContract.dataSlot() + 1));
